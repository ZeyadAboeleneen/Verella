...
--> statement-breakpoint
CREATE TABLE `store_variant_translations` (
	`id` bigint unsigned AUTO_INCREMENT NOT NULL,
	`variant_id` bigint unsigned NOT NULL,
	`locale` char(5) NOT NULL,
	`label` varchar(64) NOT NULL,
	CONSTRAINT `store_variant_translations_id` PRIMARY KEY(`id`),
	CONSTRAINT `store_variant_translations_unique` UNIQUE(`variant_id`,`locale`)
);
--> statement-breakpoint
CREATE TABLE `store_variants` (
	...
	`uuid` char(36) NOT NULL DEFAULT (uuid()),
	`product_id` bigint unsigned NOT NULL,
	`axis` varchar(16) NOT NULL DEFAULT 'volume',
	`label` varchar(64) NOT NULL,
	`sku` varchar(64),
	...
	`created_at` timestamp NOT NULL DEFAULT (now()),
	`updated_at` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `store_variants_id` PRIMARY KEY(`id`),
	CONSTRAINT `store_variants_sku_unique` UNIQUE(`sku`)
);
--> statement-breakpoint
...
CREATE TABLE `banners` (
	...
	`link2_url` varchar(512),
	`image_link_url` varchar(512),
...
CREATE TABLE `order_items` (
	...
	`variant_label_snapshot` varchar(64),
...
ALTER TABLE `store_hero_images` ADD CONSTRAINT `store_hero_images_media_id_media_id_fk` FOREIGN KEY (`media_id`) REFERENCES `media`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `store_variant_translations` ADD CONSTRAINT `store_variant_translations_variant_id_store_variants_id_fk` FOREIGN KEY (`variant_id`) REFERENCES `store_variants`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `store_variants` ADD CONSTRAINT `store_variants_product_id_store_products_id_fk` FOREIGN KEY (`product_id`) REFERENCES `store_products`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `cart_items` ADD CONSTRAINT `cart_items_variant_id_store_variants_id_fk` FOREIGN KEY (`variant_id`) REFERENCES `store_variants`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `order_items` ADD CONSTRAINT `order_items_variant_id_store_variants_id_fk` FOREIGN KEY (`variant_id`) REFERENCES `store_variants`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `store_variants_product_idx` ON `store_variants` (`product_id`);--> statement-breakpoint
